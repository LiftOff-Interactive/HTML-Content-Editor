...
_Last updated: 2026-06-23 · Current stage: **v3.0.0 foundation in progress on branch `stage-9-v3-foundation`. F1 ✅ + F4 ✅ + F2 ✅ done & verified. Save format now v3 (`kind` union). Next: F3 Phase 1 raw HTML import.**_
...
→ **NEXT: F3 Phase 1 — robust raw HTML import.** SANITIZE → WALK → WRAP. New `src/import/sanitize.js` (DOMPurify boundary — already vendored), `src/import/walk.js` (`NODE_MAP`: h1–h3/p/ul/ol/li/strong/em/u/a/img/blockquote → native ops `deltaToHtml`/`walkDelta` understand; **everything else → a new `raw-html` BlockEmbed blot** holding sanitized `outerHTML`), `src/doc-styles.js` (`window.DocStyles` — capture head `<style>`/body style/`<link>` hrefs flagged-not-fetched; inject scoped to `.ql-editor`; `buildExportHtml` concatenates `getCss` after `buildExportCSS`), `src/blots/raw-html.js` (hidden from palettes via `static widgetHidden=true` + filter in slash-command.js/toolbar.js; click opens the F2 code view seeded with its html; re-sanitize on save). Hook the `NO_EMBED` branch in `save-load.js loadHtmlProject` → `HCEImport.importArbitraryHtml`. Build via hand-built Delta + `quill.setContents(SILENT)`, **NOT** `dangerouslyPasteHTML` (drops unknown tags). New persisted field `docStyles` (additive; `migrateV2toV3` already tolerant). Verify against all four `fixtures/`: no throw, no silent content loss (fidelity report kept-vs-dropped), recognizable preview, round-trip export, clean no-JS export (zero script/on*/javascript:, no external CDN self-contained); IRCC (5.4MB) opens without freezing. See v3_master_plan §F3 Phase 1 + §5.4. **R1 (XSS):** sanitize every `innerHTML` path; consider security-reviewer before commit.
