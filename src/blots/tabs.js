(function () {
  ...
  class TabsBlot extends BaseWidgetBlot {
    ...
    _openEditModal(data) {
      ...
      // Two-column body — the single flexible region between header and footer.
      // flex:1 + min-height:0 lets it shrink inside the dialog's max-height so
      // the footer is never pushed out; columns scroll their own overflow.
      ...
      function saveCurrentRichField() {
        if (currentRichField && currentRichFieldIdx >= 0 && currentRichFieldIdx < working.tabs.length) {
          working.tabs[currentRichFieldIdx].content = currentRichField.getHtml();
        }
      ...
      function renderRight() {
        ...
        currentRichField = new RichTextField(mount, tab.content);
